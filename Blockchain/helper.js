...
export function fieldsAreValid(fields) {
  let {date, name, tutor, course, description} = fields;
  let day, month, year;
  ...
  if (!(date && name && course && tutor && description)) {
    return 'Make sure all fields are filled out';
    // return false;
  }

  // Check names are under n chars
  if (!(name.length < NAME_CHAR_LIMIT && course.length < NAME_CHAR_LIMIT)) {
    return 'Make sure your name is under 100 characters';
    // return false;
  }
  // Check vows are under n chars
  if (!(tutor.length < VOWS_CHAR_LIMIT && description.length < VOWS_CHAR_LIMIT)) {
