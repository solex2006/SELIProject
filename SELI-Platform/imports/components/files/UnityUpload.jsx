...
class UnityUpload extends Component {
  ...
  decompress(file){
    Meteor.call("Extract", file, (error, response) =>  {
      this.setState({
        decompressing: false,
        decompressResult: response,
      }, () => {
        this.refs['fileinput' + this.props.type].value = '';
        let file = this.state.file;
        file.decompressResult = this.state.decompressResult;
        this.getFileInformation(file);
      })
    });
  }
  ...
  render() {
    ...
    if (this.props.files && this.props.docsReadyYet) {
      return (
        <div>
          {
            !this.state.inProgress && !this.state.decompressing ?
